...
author: "Joseph Holler, Junyi Zhou, Peter Kedron, Drew An-Pham, Derrick Burt, Elise Chan"
date: "`r Sys.Date()`"
...
Version 3.0 \| First Created July 7, 2021 \| Updated December 16, 2023
...
#rm(acs, acs_derived, covid) #keep these variables so don't have to keep rerunning data
```
...
### Revised Missing Data table

```{r revise-data-table}
missing_data <- acs_covid %>% 
...
missing_data %>% 
  kable(
    #col.names = (""),
    #row.names = c("FIPS","State","County","County, State", ""),
    caption = ("Counties with Missing Data")
    ) %>% 
  kable_styling()
...
```{r map-white-pct, message = FALSE}
# map another variable
tm_white_pct <- tm_shape(acs_covid) +
  tm_polygons("white_pct",
    title = "Percent of White People\n(ACS 2014-2018)",
    style = "quantile",
    border.alpha = .2,
    lwd = 0.2,
    palette = "Oranges"
...
## Comparing the original and reproduced studies
...
The choropleth map (Figure 1) we made in the data part of the reproduction analysis reveals an identical spatial pattern to that of the original study's Figure 1, confirming the equivalence of our dependent variable with that of the original study.
...
Prior to Chakraborty's bivariate analysis of disability and COVID-19 incidence, we also mapped the spatial distribution of disability (Figure 2).
...
The summary statistics for the analytical part of the reproduction analysis matched perfectly with those of the original study, confirming that the reproduction uses identical original data.
